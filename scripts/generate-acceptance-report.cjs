/**
 * Generates scripts/full-system-acceptance-audit.md based on full-system-acceptance-audit.json
 * Implements all 28 report sections, PASS/FAIL matrix, Hindi/Hinglish business explanation, and final verdict.
 */
...
const mdPath = path.resolve(__dirname, 'full-system-acceptance-audit.md');

if (!fs.existsSync(jsonPath)) {
    console.error('JSON audit file not found!');
    process.exit(1);
}

const audit = JSON.parse(fs.readFileSync(jsonPath, 'utf8'));
const { metadata, phases, passFailMatrix, bugsFound, uxIssues, securityRisks, missingFeatures, workingFeatures, finalVerdict } = audit;

let md = `# Full System Acceptance Audit Report
**Varanasi Yatra — Prompt 9.11 Comprehensive Evaluation**

- **Audit Date & Time:** ${metadata.localTime} (${metadata.timestamp})
- **Target Platform:** ${metadata.platform}
- **Frontend Target:** ${metadata.frontendUrl}
- **Backend Target:** ${metadata.backendUrl}
- **Evaluator:** ${metadata.agent}
- **Overall System Verdict:** **${finalVerdict.classification}**
...
console.log(`✅ Successfully generated full markdown audit report: ${mdPath}`);
